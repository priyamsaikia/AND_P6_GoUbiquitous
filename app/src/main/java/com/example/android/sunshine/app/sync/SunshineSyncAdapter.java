...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    //wear code
    private void sendTodayData(final String low, final String high, final String info) {
        //check if same
        if (Utility.getTodayValueForSunshineWear(getContext()) != null
                && Utility.getTodayValueForSunshineWear(getContext()).equals(low)) {
            return;
        } else {
                    ...
                    .setResultCallback(new ResultCallback<DataApi.DataItemResult>() {
                        ...
                        public void onResult(@NonNull DataApi.DataItemResult dataItemResult) {
                            if (!dataItemResult.getStatus().isSuccess()) {
                                Log.d(TAG, "Failed sending data");
                            } else {
                                Log.d(TAG, "Success sending data");
                                Utility.setTodayValueForSunshineWear(getContext(), low, high, info);
                            }
                        }
                    });
